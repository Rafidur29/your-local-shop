...
(If using Windows PowerShell, adapt export to setx or use .env and python-dotenv.)

How to work without Sith (practical guidance)

Use SQLite file dev.db for all SQL interactions so backend is fully functional locally.

Mock external adapters (payment/courier) in app/adapters as deterministic classes. Add toggles in .env to switch to "real" adapters later.

Prepare Alembic migrations but leave SQL that’s compatible with SQLite and Postgres (use SQLAlchemy metadata for DDL generation). Sith can run alembic revision --autogenerate and tweak for Postgres.

For concurrency testing (stock conflict), run tests that spawn multiple HTTP checkout calls against the running local server — you can demonstrate SAGA compensation without Postgres.

Verification & validation plan integrated in development (so you collect evidence as you go)

At the end of each phase collect:

Unit test runs (pytest output & saved screenshot or CLI logs).

Integration test logs (checkout success, idempotency replays).

Health-check output (/api/health) screenshot.

OpenAPI screenshot (/docs) showing endpoints.

Scenario evidence: for each implemented scenario (e.g., guest checkout, one-click reorder), save:

empty UI/initial request (curl or Postman)

payload success response

failure & recovery (e.g., payment decline)

CI logs: attach GitHub Actions run results (passed tests).

Design traceability: map implemented endpoints → Assignment 2 tasks/CRC cards (one small table).
Keep these items in evidence/ folder and list them in your submission pdf.

Suggested milestones + time estimates for you (Chung)
...
Phase 4: 2–4 days (largest & critical)

Phase 5–6: 2–4 days

Phase 7: 1–2 days
Total rough: 2–3 weeks (intensive) — adapt to your schedule.

Minimal required test scenarios to implement early (priority)

Catalogue GET (unit) — correctness + schema.

Cart add/update → returns correct price snapshot (integration).

Reservation TTL test — create reservation & auto-expire (unit/integration).

Checkout success (idempotency) — repeated with same Idempotency-Key returns same order (integration).

Checkout conflict test — two concurrent checkout attempts for same last item → only one succeeds (concurrency test script).

Add these to tests/ and make them part of CI.

Quick checklist to hand to Sith when ready

requirements.txt & Pipfile (if used)

alembic/ skeleton & note to generate real migrations

docker-compose.yml with Postgres placeholders

docs/deployment.md describing DATABASE_URL and secret keys

app/db repository interfaces + session factory

migrations folder with one sample migration (create tables compatible with Postgres)

openapi.json and Postman collection for endpoints used by frontend

Quick inspection summary (what Rafidur has already done)

Files inspected (frontend React app): your-local-shop-main/ contains a near-complete React app implemented in TypeScript. Key completed items:

Pages & components implemented

src/pages/:

Home.tsx, Catalogue.tsx, PDP.tsx (Product detail), Cart.tsx, Checkout.tsx, Confirmation.tsx, Admin.tsx, NotFound.tsx.

src/components/:

NavBar.tsx, ProductCard.tsx.

src/stores/cartStore.ts — cart implemented in local client store (Zustand), persistence in localStorage.

src/types/ — type definitions.

src/api/ — thin API client wrappers for backend endpoints:

catalogue.ts (GET /api/products, /api/products/{sku}),

auth.ts (register/login, GET /api/customers/me),

orders.ts (POST /api/orders, GET /api/orders/{id}),

payments.ts (dev tokenization /api/payments/tokenize),

inventory.ts (GET /api/inventory/{sku}),

admin.ts (admin packing tasks, createProduct).

http.ts — core fetch wrapper. Important behaviors:

Attaches Authorization: Bearer <token> when localStorage.auth_token exists.

idemHeaders() returns X-Idempotency-Key header (frontend generates a UUID).

Public mock/ folder with sample JSON (catalogue, inventory) used as mock data / placeholder.

What the front-end does

Full UI flows (browse, add-to-cart, checkout form, confirmation).

Checkout page calls paymentsApi.tokenize and ordersApi.create (using idemHeaders()).

Admin page calls adminApi.listPacking() and adminApi.markPacked().

Conclusion: Radifur has implemented a complete UI, state management, and the client-side request shapes and headers the backend must support. The frontend currently uses local cart store (not backed by server) and expects backend endpoints to exist. That means the remaining work is largely backend API implementation + connecting the front-end to persistent cart/checkout and to admin and inventory services.
