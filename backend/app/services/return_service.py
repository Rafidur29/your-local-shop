...
class ReturnService:
    ...
    def create_return(self, order_id: int, lines: list, created_by: Optional[str] = None):
        ...
        order_lines = self.db.query(OrderLine).filter(OrderLine.order_id == order_id).all()
        ...
        for li in lines:
            ...
            if qty > getattr(ol, "qty", 0):
                raise ReturnServiceException(f"Cannot return more than purchased for {sku}")

        rr = ReturnRequest(rma_number=self._gen_rma(), order_id=order_id, status="REQUESTED", data={"created_by": created_by})
        self.db.add(rr)
        self.db.flush()

        for li in lines:
            ol = order_line_map[li["sku"]]
            rl = ReturnLine(return_id=rr.id, order_line_id=ol.id, sku=li["sku"], qty=int(li.get("qty", 1)), reason=li.get("reason"))
            # set unit_amount_cents on the return line if attribute exists
            if hasattr(rl, "unit_amount_cents"):
                price = getattr(ol, "unit_price_cents", None) or getattr(ol, "unit_price", None) or getattr(ol, "price_cents", None)
    ...
    def receive_return(self, rma_id: int, idempotency_key: Optional[str] = None):
        ...
        if idempotency_key:
            ...
            rec = self.idem_repo.begin(idem_key, operation="return_receive")

            # robust completed check
        ...
        if rr.status == "REFUNDED":
            ...
            if rr.credit_note:
                ...
                refund_resp = credit.data.get("refund") if credit.data and isinstance(credit.data, dict) else None
                return {
        ...
        for rl in rr.lines:
            if getattr(rl, "unit_amount_cents", None) is not None:
                ...
                ol = self.db.query(OrderLine).filter(OrderLine.id == rl.order_line_id).first()
                if ol:
                    price = getattr(ol, "unit_price_cents", None) or getattr(ol, "unit_price", None) or getattr(ol, "price_cents", 0)
        ...
        if invoice and invoice.data and isinstance(invoice.data, dict):
            transaction_id = invoice.data.get("payment", {}).get("transaction_id") or invoice.data.get("transaction_id")
