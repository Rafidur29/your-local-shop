...
from app.services.fulfilment_service import FulfilmentService, FulfilmentException
...
def list_packing_tasks(db: Session = Depends(get_db), limit: int = 100):
    ...
    return [{"id": t.id, "order_id": t.order_id, "status": t.status, "created_at": t.created_at.isoformat(), "assigned_to": t.assigned_to} for t in tasks]

@router.post("/packing-tasks/{task_id}/packed", summary="Mark a packing task as packed and book courier")
def pack_and_book(task_id: int, db: Session = Depends(get_db)):
    svc = FulfilmentService(db)
    try:
        shipment = svc.mark_packed_and_book(task_id)
        return {"shipment_id": shipment.id, "tracking_number": shipment.tracking_number, "status": shipment.status}
