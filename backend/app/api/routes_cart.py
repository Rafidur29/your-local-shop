from fastapi import APIRouter, Depends, Cookie, Request, Response, HTTPException
...
def get_cart(request: Request, db: Session = Depends(get_db)):
    ...
    for it in cart.items:
        items.append({"id": it.id, "sku": it.sku, "quantity": it.quantity, "price_snapshot": it.price_snapshot})
...
def add_item(payload: AddItemIn, request: Request, response: Response, db: Session = Depends(get_db)):
    cart_uuid = _get_cart_uuid_cookie(request)
    svc = CartService(db)
    cart = svc.get_or_create_cart_for_guest(cart_uuid)
    try:
