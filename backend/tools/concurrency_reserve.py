...
def order_task(i, idempotency_key, payload):
    ...
    try:
        r = requests.post(
            f"{BASE}/api/orders", json=payload, headers=headers, timeout=20
        )
...
def run_order_concurrent(workers, idempotency_key, payload):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as ex:
        futures = [
            ex.submit(order_task, i, idempotency_key, payload) for i in range(workers)
        ]
        results = [f.result() for f in futures]
        print("Results:")
        for r in results:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Concurrency test tool (reserve or orders)."
    )
